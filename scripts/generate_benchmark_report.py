...
"""
    ...
    # Find hybrid search result for main criterion
    hybrid_result = next((r for r in results if "Hybrid" in r.operation), None)
    if hybrid_result:
        status = "✅" if hybrid_result.passes_target else "❌"
        report += f"| Hybrid search P95 | < 500ms | {hybrid_result.p95_ms:.2f}ms | {status} |\n"

    # Check traversal results
    bfs_result = next((r for r in results if "BFS" in r.operation), None)
    dfs_result = next((r for r in results if "DFS" in r.operation), None)

    if bfs_result:
        status = "✅" if bfs_result.passes_target else "❌"
        report += f"| BFS Traversal P95 | < 200ms | {bfs_result.p95_ms:.2f}ms | {status} |\n"

    if dfs_result:
        status = "✅" if dfs_result.passes_target else "❌"
        report += f"| DFS Traversal P95 | < 200ms | {dfs_result.p95_ms:.2f}ms | {status} |\n"

    # Score fusion
    fusion_result = next((r for r in results if "Fusion" in r.operation), None)
    if fusion_result:
        status = "✅" if fusion_result.passes_target else "❌"
        report += f"| Score Fusion P95 | < 1ms | {fusion_result.p95_ms:.4f}ms | {status} |\n"
